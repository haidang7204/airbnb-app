...
neighborhoods = ['Midtown', 'Harlem', 'Williamsburg', 'Astoria', 'St. George']
...
data = pd.DataFrame({
    ...
    'neighborhood': np.random.choice(neighborhoods, size=length),
...
st.write('1. Use the dropdowns to select a host and neighborhood.')
st.write('2. Set your desired price range.')
st.write('3. Explore available housing rentals in the selected area.')


hosts = data['listing_name'].unique()
selected_host = st.selectbox('Select a Host', hosts)

neighborhoods = data[data['listing_name'] == selected_host]['neighborhood'].unique()
...
filtered_data = data[
    (data['listing_name'] == selected_host) &
